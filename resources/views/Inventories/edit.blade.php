<x-layout bodyClass="g-sidenav-show bg-gray-200">
    ...
    <div class="main-content position-relative bg-gray-100 max-height-vh-100 h-100 mt-3">
        <!-- Navbar -->
        <x-navbars.navs.auth titlePage='Editar Inventario'></x-navbars.navs.auth>
        <!-- End Navbar -->
                                    ...
                                    <div class="row">
                                        <div class="mb-3 col-md-12">
                                            ...
                                            <select id="" class="form-control border border-2 p-2" name="product_id">
                                                ...
                                                <option {{$product->id == $inventory->product_id ? 'selected' : ''}} value="{{$product->id}}">{{$product->name}}</option>
                                                @endforeach
                                            </select>
                                            @error('product_id')
                                        ...
                                        <div class="mb-3 col-md-12">
                                            ...
                                            <input type="number" name="stock" class="form-control border border-2 p-2" value='{{ old('stock', $inventory->stock) }}'>
                                            @error('stock')
                                                <p class='text-danger inputerror'>{{ $message }}</p>
                                            @enderror
                                        </div>
                                        <div class="mb-3 col-md-12">
                                            ...
                                            <select id="" class="form-control border border-2 p-2" name="supplier_id">
                                                ...
                                                <option {{$supplier->id == $inventory->supplier_id ? 'selected' : ''}} value="{{$supplier->id}}">{{$supplier->name}}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </div>
